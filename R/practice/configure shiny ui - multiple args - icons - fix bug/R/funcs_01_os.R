...
build_sidebar_menu <- function(ns, config){
  ...
  build_sidebar_menu_item <- function(x,y,z) { 
    
    print(menuItem(text = x, tabName = y, icon = icon(z))$children[[1]]$attribs['data-value'] %>% typeof())
    print(menuItem(text = x, tabName = y, icon = icon(z))$children[[1]]$attribs['data-toggle'] %>% typeof())
    menuItem(text = x, tabName = y, icon = icon(z))
  ...
  list_menuItems[[2]]$children[[1]]$attribs['data-value'] %>% print()
  list_menuItems[[3]]$children[[1]]$attribs['data-value'] %>% print()
...
build_sidebar_menu_subitems <- function(config_with, config_subitems){
  ...
  for (i in 1:nrow(config_with)){
    ...
    # when you attach child menuSubItems, the menuItem is stripped of data-toggle and data-value 
    # so for now we need to reset these
    # see jira item ALL-271
    list_menuItems_with[[i]]$children[[1]]$attribs['data-toggle'] <- "tab"
    list_menuItems_with[[i]]$children[[1]]$attribs['data-value'] <- tabname_i
    
    print(list_menuItems_with[[i]]$children[[1]]$attribs['data-toggle'] %>% typeof())
    print(list_menuItems_with[[i]]$children[[1]]$attribs['data-value'] %>% typeof())
  }
