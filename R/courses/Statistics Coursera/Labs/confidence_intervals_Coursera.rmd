...
```{r set-seed}
set.seed(26042019)                 # make sure to change the seed
```
...
```{r describe-sample}
samp %>% summarise(mean = mean(area), sd = sd(area), median = median(area), IQR = IQR(area), min = min(area), max = max(area))

samp %>% ggplot(aes(x=area)) + geom_histogram(binwidth=50)
...
samp %>% summarise(se = sd(area)/sqrt(n), moe = z_star_95*se, uppercf = mean(area) + moe, lowercf = mean(area) - moe)
...
sampcf <- samp %>% summarise(se = sd(area)/sqrt(n), moe = z_star_95*se, uppercf = mean(area) + moe, lowercf = mean(area) - moe)

sampcf <- sampcf %>% mutate(check = params$mu <= uppercf & params$mu >= lowercf)
sampcf
